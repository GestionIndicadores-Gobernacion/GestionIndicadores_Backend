...
class StrategyProgressService:
    ...
    def _dataset_count(metric, current_year: int) -> float:
        """
        Cuenta registros válidos del dataset seleccionado en la métrica.
        Un registro es válido si tiene el campo 'mes' definido.
        """
        if not metric.dataset_id:
            return 0.0

        # Buscar la tabla activa del dataset
    ...
    def _dataset_sum(metric, current_year: int) -> float:
        """
        Suma la columna field_name del dataset seleccionado en la métrica.
        """
