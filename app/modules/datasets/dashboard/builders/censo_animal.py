...
def build_censo_animal(fields, field_values, total):
    ...
    kpis = [
        {"label": "Población perros 2026", "value": f"{int(total_perros):,}", "sub": "estimado 2026",          "icon": "number"},
        {"label": "Población gatos 2026",  "value": f"{int(total_gatos):,}",  "sub": "estimado 2026",          "icon": "number"},
        {"label": "Municipios encuestados","value": str(mun_enc),             "sub": f"de {n_mun} municipios", "icon": "map"},
        {"label": "VTA",                   "value": f"{int(total_vta):,}",    "sub": "total",                  "icon": "check"},
    ]
    ...
    s = bar_mun(f_vta, "__vta_mun__",
                "VTA por municipio",
                "Municipios con VTA registrada", span="full")
    if s: sections.append(s)
